...
#--Set up a basemap-----------------------------------------
psbasemap $R $J $B:."Example Stereonet": -P -K > $outfile
...
echo 320/30 | stereonet | psxy $R $J -W2p/blue -M -O -K >> $outfile
...
echo S40E/30E | stereonet --poles | psxy $R $J -Sc5p -Gblue -O -K >> $outfile
...
echo 140/30NE 45NW | stereonet --rakes | psxy $R $J -Sa10p -Gred -O -K >> $outfile
...
echo 75/050 | stereonet --lines | psxy $R $J -Sc5p -Ggreen -O >> $outfile
