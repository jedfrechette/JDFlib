function varargout=plot2svg(param1,id)
%  Matlab to SVG converter
%  Only 2D plots are currently supported
%
%  Usage: plot2svg(filename,graphic handle)
%                  optional     optional
%         or
%
%         plot2svg(figuresize,graphic handle)
%                   optional    optional
%
%  Juerg Schwizer 22-May-2005
%
global PLOT2SVG_globals
global colorname
global fixcolorptr
progversion='22-May-2005';
if nargout==1
    varargout={0};
end
...
fprintf(fid,'<svg preserveAspectRatio="xMinYMin meet" width="100%%" height="100%%" viewBox="0px 0px %0.3fpx %0.3fpx" ',paperpos(3),paperpos(4));
...
axfound=0;
% Frame of figure
fprintf(fid,'  <rect x="0" y="0" width="%0.3f" height="%0.3f" fill="#e0e0e0" stroke="none" />\n',paperpos(3),paperpos(4));  % Draw white rectangle in the background of the graphic frame to cover all other graphic elements
% fprintf(fid,'<TextLine <GroupID 1><TLOrigin %0.3f" %0.3f"><TLAlignment Right><Font <FFamily Times New Roman><FSize 5><FPlain Yes><FBold No><FNoAdvance No><FPosition FNormal><FColor `White''>><String `Plot2mif %s (JS)''>>',paperpos(3),paperpos(4)-0.15,progversion);
...
if strcmp(get(ax,'PlotBoxAspectRatioMode'),'manual')
    ...
    %if axpos(3)*aspect_ratio(2) < axpos(4)*aspect_ratio(1)
    axpos(4) = axpos(3)*aspect_ratio(1)/aspect_ratio(2);
    %TODO correct position
    %else
    %axpos(3) = axpos(4)*aspect_ratio(1)/aspect_ratio(2);
    %TODO correct position
    %end
end
fprintf(fid,'  <g>\n');
if strcmp(get(ax,'Visible'),'on')
    ...
    fprintf(fid,'    <rect x="%0.3f" y="%0.3f" width="%0.3f" height="%0.3f" fill="%s" fill-opacity="%0.2f" stroke="%s" stroke-width="0.0pt" stroke-opacity="0"/>\n',...
        axpos(1)*paperpos(3), (1-(axpos(2)+axpos(4)))*paperpos(4), axpos(3)*paperpos(3), axpos(4)*paperpos(4), background_color, background_opacity, background_color);
end
fprintf(fid,'    <g>\n');
...
for i=length(axchild):-1:1
    if strcmp(get(axchild(i),'Type'),'line')
        ...
        switch marker
            case 'none';
            case '.',group=group+1;,circle2mif(fid,group,axpos,x,y,markersize/2,'none',markeredgecolorname,linewidth);
            case 'o',group=group+1;,circle2mif(fid,group,axpos,x,y,markersize,markeredgecolorname,markerfacecolorname,linewidth);
    ...
    elseif strcmp(get(axchild(i),'Type'),'image')
        ...
        %if isa(pointc,'double')
        pointc = max(min(round(double(pointc)),size(cmap,1)),1);
        %end
        CameraUpVector=get(ax,'CameraUpVector');
        filename = [PLOT2SVG_globals.basefilename num2str(PLOT2SVG_globals.figurenumber,'%03u') '.jpg'];
        ...
        imwrite(flipud(pointc),cmap,fullfile(PLOT2SVG_globals.basefilepath,filename),'jpg');
        %            lx=(size(pointc,2)*2*halfwidthx)/(axlim(3)-axlim(1))*axpos(3)*paperpos(3);
        %            ly=(size(pointc,1)*2*halfwidthy)/(axlim(4)-axlim(2))*axpos(4)*paperpos(4);
        ...
        %pointsx=(((pointx(1)-halfwidthx)-axlim(1))/axlim(3)*axpos(3)+axpos(1))*paperpos(3);
        %pointsy=(1-(((pointy(length(pointy))+halfwidthy)-axlim(2))/axlim(4)*axpos(4)+axpos(2)))*paperpos(4);
...
if strcmp(get(ax,'Visible'),'on')
    ...
    for i=1:length(axxtick)
        if ~isempty(axlabelx)
            if strcmp(get(ax,'XAxisLocation'),'top')
                label2mif(fid,grouplabel,axpos,ax,axxtick(i),(1-((axlimy(2)-axlim(2))/axlim(4)*axpos(4)+axpos(2)))*paperpos(4) - 5,convertString(axlabelx(i,:)),'Center',0,'bottom',1,paperpos,scolorname);
            else
                label2mif(fid,grouplabel,axpos,ax,axxtick(i),(1-((axlimy(1)-axlim(2))/axlim(4)*axpos(4)+axpos(2)))*paperpos(4) + 5,convertString(axlabelx(i,:)),'Center',0,'top',1,paperpos,scolorname);
            end
        end
    end
    ...
    for i=1:length(axytick)
        if ~isempty(axlabely)
            if strcmp(get(ax,'YAxisLocation'),'right')
                label2mif(fid,grouplabel,axpos,ax,((axlimx(2)-axlim(1))/axlim(3)*axpos(3)+axpos(1))*paperpos(3) + 5,axytick(i),convertString(axlabely(i,:)),'Left',0,'middle',1,paperpos,scolorname);
            else
                label2mif(fid,grouplabel,axpos,ax,((axlimx(1)-axlim(1))/axlim(3)*axpos(3)+axpos(1))*paperpos(3) - 5,axytick(i),convertString(axlabely(i,:)),'Right',0,'middle',1,paperpos,scolorname);
            end
        end
    end
...
imwrite(pict.cdata,fullfile(PLOT2SVG_globals.basefilepath,filename),'jpg');
%imwrite(pict.cdata,pict.colormap,'test.jpg','jpg');
%set(ax,'Units','inches');
set(ax,'Units','normalized');
...
if strcmp(get(ax,'XTickLabelMode'),'auto') & strcmp(get(ax,'XScale'),'linear')
    ...
    if ~isempty(indexnz)
        ...
        if round(log10(ratio(1))) ~= 0
            exptext=['&#215; 10<tspan baseline-shift="super">' num2str(-log10(ratio(1))) '</tspan>'];
            label2mif(fid,group,axpos,ax,(axpos(1)+axpos(3))*paperpos(3),(1-axpos(2))*paperpos(4)+3*fontsize,exptext,'right',0,'top',1,paperpos,font_color)           
        end
    end
end
